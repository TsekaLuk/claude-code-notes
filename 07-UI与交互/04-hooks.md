...
```mermaid
graph TD
    COMP[REPL.tsx / PromptInput.tsx\n组件层]

    subgraph hooks["钩子层"]
        VIM[useVimInput\nVim 状态机封装]
        VS[useVirtualScroll\n虚拟滚动]
        AKH[useArrowKeyHistory\n历史导航]
        TI[useTextInput\n基础输入逻辑]
        SSE[useSyncExternalStore\n滚动位置外部存储订阅]
        TOOLS[工具层\nCursor / killRing / inputFilter]
    end
...
```mermaid
flowchart TD
    A[用户按键\nInk useInput 事件] --> B[useVimInput.handleVimInput]
    B --> C[transition vimState / input / ctx\n→ TransitionResult\nNORMAL 模式]
    C --> D[execute 或 next state\nctx.setOffset / ctx.deleteRange\nctx.enterInsert ...]
    D --> E[useTextInput\n底层字符串操作]
    E --> F[onChange newValue\n→ React 状态更新 → 重渲染]
```
