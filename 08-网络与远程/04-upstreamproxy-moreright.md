...
```mermaid
flowchart TD
    subgraph upstream[src/upstreamproxy/]
        init[initUpstreamProxy<br>init.ts 调用一次]
        env[getUpstreamProxyEnv<br>子进程继承代理配置]
        relay[relay.ts]
        
        init --> check[检查 CLAUDE_CODE_REMOTE + CCR_UPSTREAM_PROXY_ENABLED]
        init --> read[readToken /run/ccr/session_token]
        init --> set[setNonDumpable prctl PR_SET_DUMPABLE=0]
        init --> dl[downloadCaBundle GET /v1/code/upstreamproxy/ca-cert]
        init --> start[startUpstreamProxyRelay wsUrl, sessionId, token]
        init --> unlink[unlink /run/ccr/session_token]
        
        start --> bl[Bun.listen]
        start --> nc[net.createServer]
        
        relay --> handle[handleData 解析 CONNECT]
        handle --> open[openTunnel]
        open --> ws[WebSocket --> encodeChunk/decodeChunk]
    end

    subgraph child[子进程 Bash/Python/MCP]
        proxy[HTTPS_PROXY=http://127.0.0.1]
    end
    
    upstream -- "127.0.0.1:ephemeral_port" --> proxy
    proxy -- "HTTP CONNECT" --> tunnel[WebSocket Tunnel Protobuf]
    tunnel --> ccr[CCR 上游代理服务 GKE 集群内]
    ccr --> target[目标 HTTPS 服务]
    
    subgraph stub[src/moreright/useMoreRight.tsx<br>公开存根 / external-stubs overlay]
        use[useMoreRight]
        ret["onBeforeQuery: async () => true<br>onTurnComplete: async () => {}<br>render: () => null"]
        use --> ret
    end
    overlay[内部构建通过 overlay 替换为真实钩子实现] -.-> stub
```
...
flowchart TD
    subgraph 容器启动
        A1[initUpstreamProxy 检查环境变量] --> A2[读取 /run/ccr/session_token]
        A2 --> A3[prctl 防 ptrace 内存转储]
        A3 --> A4[GET 下载 MITM CA 证书]
        A4 --> A5[拼接系统 CA + MITM CA]
        A5 --> A6[startUpstreamProxyRelay 绑定本地端口]
        A6 --> A7[unlink token 仅留内存]
        A7 --> A8[getUpstreamProxyEnv 返回代理环境变量]
    end

    subgraph 子进程发起 HTTPS 请求
        B1[HTTPS_PROXY 拦截连接到本地中继] --> B2[发送 HTTP CONNECT]
        B2 --> B3[中继解析 CONNECT 建立 WebSocket]
        B3 --> B4[后续字节以 Protobuf 帧传输]
        B4 --> B5[CCR 代理 MITM TLS 注入凭证转发]
    end
```
