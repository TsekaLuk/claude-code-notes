...
| ★ | `04-Agent协调/05-多Agent协作与Swarm.md` | 三层协作架构、Swarm 进程内运行器、Task 权限传递 6 条规则 |
| ★ | `04-Agent协调/06-Fork与提示词缓存优化.md` | Fork 子 Agent 机制、Prompt Cache 命中率最大化、防递归双重保险 |
| ★ | `06-服务与基础/09-system-prompt工程.md` | 分段缓存架构、SYSTEM_PROMPT_DYNAMIC_BOUNDARY、CLAUDE.md 层级发现 |
| ★ | `06-服务与基础/10-上下文压缩.md` | 四层压缩策略、AutoCompact 阈值公式、Context Collapse 与 AutoCompact 互斥 |
...
第一阶段：基础层
  ...
  06-服务与基础/09-system-prompt工程.md → 分段缓存架构、CLAUDE.md 层级发现
  06-服务与基础/10-上下文压缩.md       → 四层压缩策略与 AutoCompact 机制
...
第四阶段：查询层
  ...
  04-Agent协调/05-多Agent协作与Swarm.md → 三层协作架构与 Task 权限传递
  04-Agent协调/06-Fork与提示词缓存优化.md → Fork 机制与 Prompt Cache 优化
...
| **04 Agent 协调** | 6 篇 | `src/query.ts` `src/QueryEngine.ts` `src/coordinator/` | 查询循环驱动、会话持久化、多 Agent 协调、三层协作架构、Fork 子 Agent 与 Prompt Cache 优化 | `query()`, `QueryEngine`, `isCoordinatorMode()`, `forkAgent()` |
| **05 扩展系统** | 3 篇 | `src/plugins/` `src/skills/` | 内置插件注册、技能目录扫描、按键绑定扩展 | `registerBuiltinPlugin()`, `getDynamicSkills()` |
| **06 服务与基础** | 10 篇 | `src/services/` `src/utils/` `src/bootstrap/` | API 通信、认证体系、全局状态、token 成本计算、System Prompt 分段缓存架构、四层上下文压缩策略 | `getAnthropicClient()`, `getAuthTokenSource()`, `AppState`, `SYSTEM_PROMPT_DYNAMIC_BOUNDARY` |
