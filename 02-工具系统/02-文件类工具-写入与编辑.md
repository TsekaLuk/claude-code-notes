...
FileWriteTool                        FileEditTool
     │                                    │
     ├─ utils/file.js                     ├─ utils/file.js
     │  (writeTextContent,                │  (writeTextContent,
     │   getFileModificationTime)         │   getFileModificationTime)
     ├─ utils/diff.js                     ├─ utils/diff.js
     │  (getPatchForDisplay,              │  (countLinesChanged,
     │   countLinesChanged)               │   getPatchForEdit)
     ├─ FileEditTool/constants.js         ├─ utils/fileRead.js
     │  (FILE_UNEXPECTEDLY_MODIFIED_ERROR)│  (readFileSyncWithMetadata)
     │                                    ├─ ./utils.js
     │                                    │  (findActualString,
     │                                    │   preserveQuoteStyle,
     │                                    │   getPatchForEdit)
     ...
     │   getLspServerManager() → changeFile/saveFile  │
     │                                                │
...
```
Phase 1: validateInput() — 无副作用验证
  ├─ checkTeamMemSecrets()  → 拒绝向团队记忆文件写入密钥
  ├─ old_string === new_string? → 拒绝（无意义操作）
  ├─ denyRule 检查 → 被权限规则阻止？
  ├─ UNC 路径检查 → 跳过（安全，让权限检查处理）
  ├─ 文件大小 > 1 GiB? → 拒绝（防 OOM）
  ├─ 文件不存在 + old_string 非空? → 拒绝（提示相似文件）
  ├─ 文件是 .ipynb? → 拒绝（引导用 NotebookEditTool）
  ├─ readFileState 中无记录? → 拒绝（未读取）
  ├─ 文件时间戳比读取时间新? → 拒绝（文件已被修改）
  ├─ findActualString() 找不到? → 拒绝
  └─ 多处匹配 + replace_all=false? → 拒绝（歧义）

Phase 2: call() — 原子性写入
  ├─ discoverSkillDirsForPaths() → 发现新技能目录
  ├─ diagnosticTracker.beforeFileEdited() → 诊断预备
  ├─ mkdir() → 确保父目录存在
  ├─ fileHistoryTrackEdit() → 备份预编辑内容
  ├─ readFileForEdit() → 同步重读文件（原子性保障）
  ├─ 时间戳二次检查（防并发修改）
  ├─ findActualString() + preserveQuoteStyle() → 规范化
  ├─ getPatchForEdit() → 计算替换后内容和 diff
  ├─ writeTextContent() → 写入磁盘
  ├─ lspManager.changeFile() + saveFile() → 通知 LSP
  ├─ notifyVscodeFileUpdated() → 通知 VSCode diff 视图
